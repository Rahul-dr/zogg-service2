...
public class VoucherServiceImpl implements VoucherService {
    ...
    public VoucherResponseDto getVoucherById(String voucherId) {
        ...
        VoucherResponseDto voucherResponseDto = VoucherMapper.INSTANCE.toDto(voucherCollection);

        BrandCollection brandCollection =
                brandCollectionRepository.findById(voucherCollection.getBrandId()).orElse(null);

        if (brandCollection != null) {
            voucherResponseDto.setBrandName(brandCollection.getName());
            voucherResponseDto.setBrandDescription(brandCollection.getDescription());
            voucherResponseDto.setBrandWebsite(brandCollection.getWebsiteUrl());
            voucherResponseDto.setBusinessCategory(brandCollection.getBusinessCategory());
        }

        // Check if voucher is redeemed by getting coupon details
        CouponCollection coupon =
                couponCollectionRepository.findByVoucherId(voucherId).orElse(null);

        if (coupon != null) {
            voucherResponseDto.setCouponCode(coupon.getCouponCode());
            voucherResponseDto.setRedeemed(true);
        }
